...
import safe.kernel.flash.common.extensions.ByteArray.toHex
import safe.kernel.flash.common.extensions.ExtendedFile.inputStream
...
class SlotViewModel(
    ...
    private fun resolveSourcePath(uri: Uri?): String {
        if (uri == null) return "unknown"
        return when {
            uri.scheme == "file" -> File(uri.path!!).absolutePath
            uri.toString().contains("com.android.externalstorage.documents") -> {
                val docId = uri.lastPathSegment ?: return Uri.decode(uri.toString())
    ...
    fun backup(context: Context) {
        launch {
            ...
            HistoryManager.record(HistoryEntry.create("备份分区 -> /sdcard/ReKernelFlasher/backups/$now"))
            SharedViewModels.mainViewModel.markRefreshNeeded()
        }
    }
...
//                    Log.e(TAG, "Error: ${result.stderr.joinToString("\n")}")
        ...
        launch {
            ...
            try {
                if (driver.exists()) {
                    ...
                    _wasFlashSuccess.value = true
                    val srcPath = resolveSourcePath(flashUri)
                    HistoryManager.record(HistoryEntry.create("刷写 $srcPath -> ${boot.absolutePath} (KernelSU 驱动)"))
                } else {
    ...
    fun flashImage(context: Context, uri: Uri, partitionName: String) {
        ...
        launch {
            ...
            AutoBackupManager.backup(context, partitionName, slotSuffix)
            ...
            try {
                if (image.exists()) {
                    ...
                    _wasFlashSuccess.value = true
                    val srcPath = resolveSourcePath(flashUri)
                    val dest = PartitionUtil.findPartitionBlockDevice(context, partitionName, slotSuffix)?.path ?: "$partitionName$slotSuffix"
                    HistoryManager.record(HistoryEntry.create("刷写 $srcPath -> $dest"))
                } else {
